...
public class JoyStickCtrl : MonoBehaviour, IPointerUpHandler, IPointerDownHandler, IDragHandler
{
    ...
    private Quaternion cam;                 //플레이어 회전값(회전)
    private bool isTouch = false;           //터치 중인지
    public float distance;                 //조이스틱 배경과 핸들사이의 거리차
    private float angle;                    //조이스틱 배경과 마우스 사이의 각도
    #endregion

    public ButtonEvent anim;
...
//            Debug.Log(animator.GetBool("ATTACK"));
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        movePos = Camera.main.transform.TransformDirection(movePos);
        //Debug.Log(movePos);
        //플레이어의 회전 값
        //atan을 이용해서 각도를 구함
        //value.x = 벡터의 x 방향
        ...
        rot.eulerAngles = new Vector3(0, (Mathf.Atan2(value.x, value.y) * Mathf.Rad2Deg)+Camera.main.transform.rotation.eulerAngles.y, 0);
        //rot = Quaternion.LookRotation();
        
        //rot.eulerAngles.Set(0,rot.eulerAngles.y+90,0);
